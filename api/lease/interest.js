...
// Flow: validate → look up listing → deduplicate → record → email outgoing tenant.
...
module.exports = async (req, res) => {
  ...
  try {
    ...
    // ── 5. Email the outgoing tenant ──────────────────────────────────────────
    const firstName  = incoming_name ? incoming_name.split(' ')[0] : 'A student';
    const uniLine    = incoming_uni  ? ` studying at ${incoming_uni}` : '';
    ...
    const listingTitle = listing.title || `${listing.suburb} — ${listing.property_type}`;
 
    await resend.emails.send({
      from:     'UniRenter <noreply@unirenter.com.au>',
      to:       listing.email,
      // TODO S27: swap reply-to for listing-{ref}@unirenter.com.au once catch-all MX routing configured
      reply_to: 'noreply@unirenter.com.au',
      bcc:      'benjcarey75@gmail.com',
      subject:  `Someone's interested in your ${cityTitle} listing (${listing_ref})`,
      html: `
        <div style="font-family:Inter,sans-serif;max-width:560px;margin:0 auto;color:#1a1a1a;">
          <div style="background:#0d1f2d;padding:24px 32px;border-radius:12px 12px 0 0;">
            ...
            <div style="font-size:13px;color:#7a96aa;margin-top:4px;">Break Lease Board · ${cityTitle}</div>
          </div>
          <div style="background:#f9f9f9;padding:28px 32px;border-radius:0 0 12px 12px;border:1px solid #e8e8e8;border-top:none;">
            ...
            <div style="background:#fff;border:1px solid #e0e0e0;border-radius:10px;padding:16px 20px;margin-bottom:20px;">
              ...
              <div style="font-size:13px;color:#777;">${listing_ref} · ${cityTitle} · ${rentDisplay}</div>
            </div>
            <p style="color:#555;font-size:14px;line-height:1.7;margin:0 0 16px;">
              UniRenter will facilitate the next step. <strong>Keep all contact through the platform</strong> — 
            ...
            <div style="background:#fff8e6;border:1px solid #f0d080;border-radius:10px;padding:16px 20px;margin-bottom:20px;">
              <div style="font-weight:700;font-size:13px;color:#8a6000;margin-bottom:8px;">🤠 Cob safety reminder</div>
              <ul style="margin:0;padding:0 0 0 18px;font-size:13px;color:#555;line-height:1.9;">
                ...
                <li>Get <strong>written landlord consent</strong> — verbal agreement is not enough</li>
            ...
            <div style="margin-top:24px;padding-top:20px;border-top:1px solid #e8e8e8;font-size:11px;color:#aaa;">
              UniRenter · unirenter.com.au · Free for students, always.<br>
