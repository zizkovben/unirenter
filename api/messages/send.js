...
// Sends a message from sender to recipient, persisted in Supabase
...
// Safety: block obvious scam/financial patterns
const BLOCKED_PATTERNS = [
  /\b\d{4}[\s-]?\d{4}[\s-]?\d{4}[\s-]?\d{4}\b/,  // card numbers
  /\b\d{6}[\s-]\d{6,10}\b/,                         // BSB/account
...
module.exports = async function handler(req, res) {
  ...
  // Verify sender is a real verified profile
